...
(function() {
  function buildPrompt(tweet, persona, openlabsConfig, shouldPromote) {
    const exp = Array.isArray(persona.expertise) ? persona.expertise.join(", ") : persona.expertise;
    let sys = `You are ${persona.name}, ${persona.title} @ ${persona.company}. Tone: ${persona.tone}. Expertise: ${exp}.
` + `Rules: <150 chars, no hashtags, natural/intellectual, end with period(.).`;
    if (shouldPromote) {
      sys += ` Mention ${persona.company} (${openlabsConfig.url}) contextually, no sales pitch.`;
    } else {
  ...
  async function generateReply(apiKey, tweet, persona, openlabsConfig, shouldPromote) {
    if (!apiKey) {
    ...
    const { systemInstruction, userPrompt } = buildPrompt(tweet, persona, openlabsConfig, shouldPromote);
    try {
